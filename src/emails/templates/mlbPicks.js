/**
 * MLB Maximus's Picks — model-driven picks and edges (light mode).
 *
 * @param {object} data
...
import { MlbEmailShell, mlbHeroBlock, mlbSectionHeader, mlbGlassCard, mlbDividerRow } from '../MlbEmailShell.js';
...
export function renderHTML(data = {}) {
  ...
  <td style="padding:8px 10px;font-size:13px;color:#1a1a2e;font-weight:600;font-family:'DM Sans',Arial,sans-serif;">${name}</td>
  <td style="padding:8px 10px;font-size:13px;color:#c41e3a;font-weight:700;text-align:center;font-family:'DM Sans',Arial,sans-serif;">${pct}</td>
...
<tr>
  <td style="padding:0 24px 14px;" class="section-td">
    ...
    <div style="margin-top:8px;">
      <a href="https://maximussports.ai/mlb/insights" style="font-size:12px;color:#c41e3a;text-decoration:none;font-weight:600;font-family:'DM Sans',Arial,sans-serif;">Full ATS board &rarr;</a>
    </div>
  </td>
</tr>`;
  }
  ...
  if (worstAts.length > 0) {
    ...
    fadeSection = mlbGlassCard({
      ...
      body: `<strong style="color:#1a1a2e;">${bottom.name || bottom.team}</strong> is covering at only ${pct || 'a low rate'} ATS. When the public piles on, value often hides on the other side.`,
    });
  }
  ...
  if (oddsSource.length > 0) {
    const lineRows = oddsSource.slice(0, 4).map(g => {
      ...
      return `<tr><td style="padding:8px 0;border-bottom:1px solid #e8ecf0;">
        <p style="margin:0;font-size:14px;font-weight:600;color:#1a1a2e;font-family:'DM Sans',Arial,sans-serif;">${matchup}</p>
        ${details ? `<p style="margin:3px 0 0;font-size:12px;color:#8a94a6;font-family:'DM Sans',Arial,sans-serif;">${details}</p>` : ''}
      </td></tr>`;
    }).join('');
...
<tr>
  <td style="padding:0 24px 14px;" class="section-td">
    ...
    <div style="margin-top:8px;">
      <a href="https://maximussports.ai/mlb/picks" style="font-size:12px;color:#c41e3a;text-decoration:none;font-weight:600;font-family:'DM Sans',Arial,sans-serif;">Full picks board &rarr;</a>
    </div>
  </td>
</tr>`;
  }
  ...
  if (pinnedTeams.length > 0) {
    ...
    if (inLeaders.length > 0) {
      spotBody = `Your team <strong style="color:#1a1a2e;">${inLeaders[0].name}</strong> appears in today\u2019s ATS leaders. Strong position \u2014 monitor line movement before first pitch.`;
    } else {
    ...
    teamSpotlight = mlbGlassCard({ label: 'YOUR TEAMS', headline: 'ATS Spotlight', body: spotBody });
  }
