...
export function getSubject(data = {}) {
  ...
  if (top) return `Maximus ATS: ${top.name || top.team} is your edge today`;
  return 'Maximus: Odds & ATS Intel — Today\'s lines and edges';
}
...
export function renderHTML(data = {}) {
  ...
  if (bestAts.length > 0) {
    atsRows = bestAts.slice(0, 5).map((team, i) => {
      ...
      const rankRow = rankingsTop25.find(r => (r.name || '').toLowerCase().includes(name.toLowerCase().split(' ').pop()));
  ...
  if (worstAts.length > 0) {
    ...
    upsetBody = 'No standout underperformers today. The market looks efficient — Maximus is watching for movement.';
  }

  // ── Games with odds context
  const gamesWithOdds = scoresToday.filter(g => g.spread || g.overUnder || g.moneylineHome).slice(0, 3);
  let oddsRows = '';
  if (gamesWithOdds.length > 0) {
    oddsRows = gamesWithOdds.map(g => {
      ...
      const details = [spread, ou].filter(Boolean).join(' &nbsp;·&nbsp; ');
      return `<tr>
  ...
  if (pinnedTeams.length > 0) {
    ...
    const inLeaders = pinnedTeams.filter(t =>
      bestAts.some(a => (a.name || '').toLowerCase().includes((t.name || '').toLowerCase().split(' ').pop()))
    );
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" width="100%"
      ...
      <tr>
        <td style="padding:10px 12px 12px;">
          <a href="https://maximussports.ai" style="font-size:11px;color:#3C79B4;text-decoration:none;font-weight:600;">Full ATS board →</a>
        </td>
      </tr>
    </table>
  </td>
...
<tr>
  <td style="padding:0 32px 16px;" class="section-pad">
    <table role="presentation" cellpadding="0" cellspacing="0" width="100%"
      ...
      <tr>
        <td style="padding:10px 12px 12px;">
          <a href="https://maximussports.ai" style="font-size:11px;color:#3C79B4;text-decoration:none;font-weight:600;">Full odds board →</a>
        </td>
      </tr>
    </table>
  </td>
...
${pinnedAtsBody ? sectionCard({
  ...
  return EmailShell({
    content,
    previewText: `ATS leaders, line movement, and upset watch for ${today}. Maximus has your edge.`,
  });
}
...
export function renderText(data = {}) {
  ...
  const lines = [
    ...
    `${today}`,
    ``,
    `Hey ${name}, the lines are moving. Here's Maximus's read.`,
    ``,
    `ATS LEADERS`,
    ...(best.length > 0
