...
export function getSubject(data = {}) {
  ...
  if (pinnedTeams.length === 0) return 'Maximus Sports: Team Alerts — Today';
...
export function renderHTML(data = {}) {
  ...
  if (pinnedTeams.length === 0) {
    const content = `
${heroBlock({ line: `Your Teams${greetingName}: Stay Sharp Today`, sublabel: today })}
${sectionCard({
      pillLabel: 'INTEL',
      pillType: 'intel',
  ...
  // ── Headlines filtered to pinned teams
  ...
  const heroLine = pinnedTeams.length === 1
    ? `${pinnedTeams[0].name}${greetingName} \u2014 Here\u2019s today\u2019s intel.`
    : `Your ${pinnedTeams.length} Teams${greetingName} \u2014 Here\u2019s today\u2019s intel.`;
