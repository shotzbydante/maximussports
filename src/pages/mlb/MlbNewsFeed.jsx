/**
 * MLB News Feed — premium intelligence/media surface.
 *
 * Structure mirrors NCAAM NewsFeed patterns adapted for baseball:
 *   - Tab system: MLB Intel / Betting Intel
 *   - Content modes: All / Videos / Stories
 *   - League & Division filtering
 *   - Hero zone with featured video + article
 *   - Video grid
 *   - Headlines stream
 *   - Betting intel tab with MLB betting videos + headlines
 */
...
const DIVISION_ORDER = ['All', 'AL East', 'AL Central', 'AL West', 'NL East', 'NL Central', 'NL West'];
...
function detectDivision(title) {
  ...
  // Check abbreviations for short team refs
  for (const t of MLB_TEAMS) {
    if (lower.includes(t.abbrev.toLowerCase())) return t.division;
  }
  return null;
}
...
function getPublisherChip(source) {
  ...
  return { lines: [source?.toUpperCase() || 'NEWS'], bg: 'linear-gradient(135deg, #555 0%, #777 100%)' };
}
...
export default function MlbNewsFeed() {
  ...
  useEffect(() => {
    fetchMlbHeadlines()
      .then(d => setHeadlines((d.headlines ?? []).map(enrichItem)))
      .catch(() => {})
      .finally(() => setHeadlinesLoading(false));
  }, []);

  // Fetch betting content when tab switches
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (newsRes.status === 'fulfilled') setBettingNews((newsRes.value.items ?? []).map(enrichItem));
    }).finally(() => setBettingLoading(false));
  }, [tab, bettingVideos.length, bettingNews.length]);

  // Filter videos by division
  const filteredVideos = useMemo(() => {
    if (divisionFilter === 'All') return videos;
    const leagueMatch = divisionFilter === 'AL' || divisionFilter === 'NL';
    return videos.filter(v => {
      const div = detectDivision(v.title);
      const league = div?.startsWith('AL') ? 'AL' : div?.startsWith('NL') ? 'NL' : null;
      if (leagueMatch) return league === divisionFilter;
      return div === divisionFilter;
    });
  }, [videos, divisionFilter]);

  // Filter headlines by division
  const filteredHeadlines = useMemo(() => {
    if (divisionFilter === 'All') return headlines;
    const leagueMatch = divisionFilter === 'AL' || divisionFilter === 'NL';
    return headlines.filter(h => {
      if (leagueMatch) return h.league === divisionFilter;
      return h.division === divisionFilter;
    });
  }, [headlines, divisionFilter]);

  const heroVideo = filteredVideos[0] || null;
  const gridVideos = filteredVideos.slice(1, 7);
  const heroHeadline = filteredHeadlines[0] || null;

  const divFilters = useMemo(() => {
    if (divisionFilter === 'All' || divisionFilter === 'AL' || divisionFilter === 'NL') return DIVISION_ORDER;
        ...
        <>
          ...
          {(contentMode === 'all' || contentMode === 'videos') && heroVideo && (
            <section className={styles.heroZone}>
              ...
              {heroHeadline && contentMode === 'all' && (
                <div className={styles.featuredArticle}>
                  <LogoChip source={heroHeadline.source} />
                  <div className={styles.featuredBody}>
                    {heroHeadline.signal && (
                      <span className={`${styles.signalTag} ${styles[heroHeadline.signal.cls] || ''}`}>
                        {heroHeadline.signal.tag}
                      </span>
                    )}
                    <span className={styles.featuredTime}>{heroHeadline.time}</span>
                    {heroHeadline.link ? (
                      <a href={heroHeadline.link} target="_blank" rel="noopener noreferrer"
                        className={styles.featuredTitle}>{heroHeadline.title}</a>
                    ) : (
                      <span className={styles.featuredTitle}>{heroHeadline.title}</span>
                    )}
                  </div>
                </div>
              )}
            ...
            <section className={styles.section}>
              ...
              {headlinesLoading ? (
                ...
                <div className={styles.headlineStream}>
                  {filteredHeadlines.slice(contentMode === 'stories' ? 0 : 1, 20).map((item, i) => (
        ...
        <>
          <div className={styles.bettingIntro}>
            ...
            <p className={styles.bettingSubtitle}>
              Picks, projections, odds movement, and market analysis from trusted betting sources
            </p>
          </div>
          ...
          {bettingLoading ? (
            ...
            <>
              ...
              {bettingVideos.length === 0 && bettingNews.length === 0 && (
                <p className={styles.emptyMsg}>Betting intel is loading. Check back shortly.</p>
              )}
            </>
          )}
        </>
