/**
 * MLB Games — ranked daily game slate with live intelligence.
...
import styles from './MlbGames.module.css';
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const r = await fetch(`/api/mlb/live/games?status=${statusParam}&sort=${sort}`, { signal: controller.signal });
        clearTimeout(timer);
        if (!r.ok) return;
