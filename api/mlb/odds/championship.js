/**
 * GET /api/mlb/odds/championship — MLB World Series winner odds by team slug.
 * Mirrors the pattern of api/odds/championship for NCAAB.
 * Aggregates across ALL bookmakers; maps outcome names to MLB team slugs.
 */

import { getJson, setJson, MAX_TTL_SECONDS } from '../../_globalCache.js';
import { MLB_TEAMS } from '../../../src/sports/mlb/teams.js';

const KV_KEY = 'odds:championship:mlb:v1';
const TTL_SECONDS = Math.min(60 * 60, MAX_TTL_SECONDS);
const ODDS_API_SPORT = 'baseball_mlb_world_series_winner';
...
function buildAliasMap() {
  ...
  for (const t of MLB_TEAMS) {
    ...
    m[lower] = t.slug;
    // e.g. "New York Yankees" → slug "nyy"
    const parts = lower.split(' ');
    if (parts.length >= 2) {
      m[parts[parts.length - 1]] = t.slug; // mascot alone
      m[parts.slice(1).join(' ')] = t.slug; // without city
    }
