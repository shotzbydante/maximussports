 ...
 *
 * The get_following() database function:
...
function normalizeRow(row) {
  ...
  return {
    ...
    followStatus: row.follow_status || 'following',
  };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data, error } = await sb.rpc('get_following');

    if (error) {
      console.error('[following] rpc error:', error.message, error.code);
      if (error.code === '42883') {
        return res.status(500).json({ error: 'get_following function not found — run docs/follower-list-rpcs.sql' });
      }
      const status = error.message?.includes('JWT') ? 401 : 500;
      return res.status(status).json({ error: error.message || 'Query failed' });
    }

    const following = (data || []).map(normalizeRow);
